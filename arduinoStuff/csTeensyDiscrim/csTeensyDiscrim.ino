/*
  csTeensyDiscrim
  Main Teensy Script For Discrim Task. Assumes a 3.2,3.5/6 teensy for speed sake.
  Will work with any MC with hardware serial, you may need to increase loopDelta.
...
const int waterLPin = 13;   // Engage Water
const int waterRPin = 13;   // Engage Water
...
void setup() {
  ...
  motionSerial.begin(9600);
  cueSerialOut.begin(9600);
  while (!Serial);
...
int lookForSerialState() {
  ...
  if (Serial.available() > 0) {
    ...
    lastState = pyState;
    stateChange = 1;  
    // you can use this! to trigger a header etc.; i don't but that was it's intent
  }
...
void spitData() {
  Serial.print("data,"); Serial.print(trialTimeMicro); Serial.print(','); Serial.print(stateTimeMicro);
  Serial.print(','); Serial.print(readOrient); Serial.print(','); Serial.print(curState);
  Serial.print(','); Serial.print(lickSensorL); Serial.print(','); Serial.print(lickSensorR);
  Serial.println();
}
